...
class CssConverter extends React.Component {
  ...
  searchElemData = (item: string, style: string) => {
    ...
    let propTailwind = /([\w\/]*\-?\w?)-([\w\/\[\]\-]*)/.exec(item);
    let objItem;
    if(this.typeStyle == style){
      ...
      if(this.typeStyle == "css" && (objItem = this.dataArr.find((obj: any) => obj["css"] == `color: ${propCSS![2]};`))){
        ...
        if(objItem) td.innerText = (objItem[style]) ? Object.keys(tempObj).find((elem: string) => tempObj[elem] == propTailwind![1]) + ": " + regCSS.exec(objItem[style])![2] + ";" : "";
        else td.innerText = "";
  ...
  convertData = async () => {
    if(this.dataField != '' && this.typeStyle != ''){
      this.setState({
        blockTable: true,
      });
